...
function m_emotions_replace($s) {
	...
	foreach ($emotions as $a => $b) {
		...
		$s = str_replace($x,'<img title=\''.$p.'\' src=emoticons/'.$a.'.gif>',$s);
	}
	return $s;
}
...
function loadPage($html) {
	global $tpl;
//	$html = $tpl->get_tpl($fileName);
	$tpl->parse_tpl($tpl->get_tpl('header'));
	$tpl->parse_tpl($html);
	$tpl->parse_tpl($tpl->get_tpl('footer'));
...
function m_get_data($type,$v,$field = '') {
	...
	elseif ($type == 'ALBUM') {
		...
		if (!$cached['album']['album_'.$v][$field]) {
			if ($v == 0 && $field == 'album_name') return "Updating";
	...
	elseif ($type == 'SINGER') {
		...
		if ($field == 'singer_name') {
			...
			if (!$c_name) {
				if ($v == -1) $c_name = "Unknown (US)";
				elseif ($v == -2) $c_name = "Unknow (Global)";
...
function m_build_mail_header($to_email, $from_email) {
	...
	$headers .= 'X-MimeOLE: Produced By AznSoft'. $CRLF;
