...
function admin_viewpages($ttrow,$n,$pg){
	...
	$pgt = $pg-1;
	if ($pg<>1) $html.="<a class=pagelink href=$link onfocus=this.blur() title ='Front page'><b>&laquo;&laquo;</b></a> <a class=pagelink href=$link&pg=$pgt onfocus=this.blur() title='Previous'><b>&laquo;</b></a> ";
	for($l = 0; $l < $ttrow/$n; $l++) {
		$m = $l+1;
	...
	$pgs = $pg+1;
	if ($pg<>$m) $html.="<a class=pagelink href=$link&pg=$pgs onfocus=this.blur() title='Next'><b>&raquo;</b></a> <a class=pagelink href=$link&pg=$m onfocus=this.blur() title='Last'><b>&raquo;&raquo;</b></a> ";
	$html.="</td></tr></table>";
	return $html;
}

function acp_quick_add_singer_form() {
	$html = "<input name=new_singer class='form-control' > <select class='form-control' name=singer_type>".
		"<option value=1 selected>US Singer</option>".
		"<option value=2>International Singer</option>".
...
function acp_album_list($id = 0, $add = false) {
	...
	$html = "<select class='form-control' name=album>";
	if ($add) $html .= "<option value=dont_edit".(($id == 0)?" selected":'').">Don't Edit</option>";
	$html .= "<option value=0".(($id == 0 && !$add)?" selected":'').">Unknown</option>";
...
function acp_singer($id = 0, $add = false) {
	...
	$html = "<select class='form-control' name=singer>";
	if ($add) $html .= "<option value=dont_edit".(($id == 0)?" selected":'').">Don't Edit</option>";
	$html .= "<option value=-1".((($id == -1 || $id == 0) && !$add)?" selected":'').">Unknown (USA)</option>";
	$html .= "<option value=-2".(($id == -2 && !$add)?" selected":'').">Unknown (Global)</option>";
	//$html .= "<option value=-3".(($id == -3 && !$add)?" selected":'').">Unknow (Band)</option>";
	while ($r = $mysql->fetch_array($q)) {
...
function acp_singer_type($i) {
	$html = "<select class='form-control' name=singer_type>".
		"<option value=1".(($i==1)?' selected':'').">US Singer</option>".
		"<option value=2".(($i==2)?' selected':'').">International Singer</option>".
	"</select>";
	return $html;
}
function acp_cat($id = 0, $add = false) {
	...
	$html = "<select class='form-control' name=cat>";
	if ($add) $html .= "<option value=dont_edit".(($id == 0)?" selected":'').">Don't Edit</option>";
	if ($arr) 
...
function acp_maincat($id) {
	...
	$html = "<select class='form-control' name=sub>";
...
function acp_check_permission($t) {
	...
	if ($level == 2 && !$mod_permission[$t]) die('<center>You dont have permission to view this page !</center>');
}
?>